...
void SPI_Flash_ReadUnfixed(uint8_t*,uint32_t,uint32_t);    //读取不固定长度数据
...
void SPI_Flash_WritePage(uint8_t*,uint32_t,uint16_t);      //写入页(256Bytes),写入长度不超过256字节
...
/*
  ...
  *         WriteLength ：写入数据长度，必须小于等于SPI_FLASH_PerWritePageSize(256Bytes)
	* @retval None
*/
void SPI_Flash_WritePage(uint8_t* pWriteBuffer, uint32_t WriteAddr, uint16_t WriteLength)
{
	//检测flash是否处于忙碌状态
	SPI_Flash_WaitForWriteEnd();
...
/*
  ...
  *         WriteLength ：写入数据长度
	* @retval None
*/
void SPI_Flash_WriteUnfixed(uint8_t* pWriteBuffer, uint32_t WriteAddr, uint32_t WriteLength)
{
...
/*
  ...
  *         ReadLength ：读取数据长度
	* @retval None
*/
void SPI_Flash_ReadUnfixed(uint8_t* pReadBuffer, uint32_t ReadAddr, uint32_t ReadLength)
{
	//检测flash是否处于忙碌状态
	SPI_Flash_WaitForWriteEnd();
